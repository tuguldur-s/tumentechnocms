<template>
	<div class="scrollable" v-loading.fullscreen.lock="loading">
		...
		<div class="page-ecommerce-dashboard">
			<div class="table-box card-base card-outline">
				<table class="styled striped hover">
					...
					<tbody>
						<tr v-for="item in orders" :key="item.id" :style="{'cursor': 'pointer'}" @click="goOrder(item.reportNumber)">
							<td>
							...
							<td>
								<div class="item-box item-location">
									...
									<p class="m-0 o-060">{{item.reportNumber}}</p>
								</div>
							</td>
						</tr>
					</tbody>
        ...
        <el-dialog title="Тайлан илгээх" :visible.sync="modals.newReport">
            <el-date-picker style="width: 100%;" v-model="newReportDate" type="date" placeholder="Тайлангийн өдрөө сонгоно уу"> </el-date-picker>
            <div class="table-box card-base card-outline">
				<table class="styled striped hover">
					<thead>
						<tr>
					...
					<tbody>
                        <tr v-for="(item, index) in addProducts" :key="index">
                            ...
                            <td>
                                <el-button type="danger" circle="" size="mini" icon="el-icon-delete" @click="removeRow(index)"></el-button>
                            </td>
                        </tr>
                        <tr>
			...
			<div style="height: 1px; background-color: grey;" class="mt-20"></div>
            <div class="mt-10">
...
export default {
	...
	computed: {
		...
		totalCash() {
            ...
            this.addProducts.forEach(element => {
                total += element.qty * element.sale_price;
            });
            return total;
		},
        totalCount () {
	...
	methods: {
        addRowToProduct(item) {
            ...
            if(!c) {
                var a = {id: item.id, model: item.model, name: item.name, qty: 1, sale_price: item.sale_price, image: item.image, type: item.type};
                this.addProducts.push(a);
            }   
        },
        addRow() {
		...
		SendToReport() {
			if(this.newReportDate == null || this.addProducts.length == 0) {
                ...
                var per = true;
                this.allproducts.forEach(all => {
                    this.addProducts.forEach(add => {
                        if(add.model == all.model && add.type == all.type) {
                            if(add.qty > all.qty) {
                                this.$notify({
                                    title: 'Амжилтгүй',
                                    dangerouslyUseHTMLString: true,
                                    message: `<strong>${add.model}</strong> үлдэгдэл хүрэлцэхгүй байна`,
                                    type: 'error'
                                });
                                per = false;
                            }
                ...
                if(per) {
                    ...
                    if(token) {
                        this.$axios({
                            method: 'post',
                            url: rts.$appUrl +`/api/order/add-customer-report`,
                            headers: {
                        ...
                        .then(function(data){
                            ...
                            if(data.data.result == 'success') {
                                ...
                                rts.$notify({
                                    title: 'Амжилтгүй',
                                    dangerouslyUseHTMLString: true,
                                    message: `Тухайн өдөр тайлан бүртгэгдсэн байна`,
                                    type: 'error'
                                });
                            }
                        }).catch(error => {
		...
		getOrders(datetime) {
			...
			if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/order/customer-reports`,
					headers: {
						"Authorization": `Bearer ${token}`
					}, 
					data: {
