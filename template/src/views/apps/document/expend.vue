...
export default {
	...
	methods: {
		toSend() {
			this.$prompt('Имэйл хаягаа оруулна уу', 'Зарлагын баримт илгээх', {
			...
			inputErrorMessage: 'Invalid Email'
				...
				if(token) {
					this.$axios({
						method: 'post',
						url: rts.$appUrl +`/api/expend/send-expend`,
						headers: {
        ...
        getInvoice() {
			...
			if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/expend/get-expend-done`,
					headers: {
						"Authorization": `Bearer ${token}`
					},
                    data: {
                        expendNumber: rts.invoiceNumber
                    }
				})
				.then(function(data){
					if (data.data.result == 'success') {
