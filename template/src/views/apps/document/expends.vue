	...
	<div class="page-ecommerce-dashboard scrollable" :style="{'max-height': '87%'}">
		<div class="table-box card-base card-outline" :style="{'overflow': 'auto'}">
			<table class="styled striped hover">
				...
				<tbody>
					<tr v-for="(item, index) in invoices" :key="index" :style="{'cursor': 'pointer'}">
						...
						<td>
							<el-button type="danger" icon="el-icon-delete" size="small" @click="deleteExpend(item.expend_number, index)" circle></el-button>
						</td>
					</tr>
				</tbody>
			</table>
...
export default {
	...
	methods: {
		deleteExpend(ordernumber, index) {
			var user = JSON.parse(localStorage.user);
			this.$confirm(`<b>${ordernumber}</b> зарлагын баримтыг устгахдаа итгэлтэй байна уу?`, 'Анхааруулга', {
				...
				if(token) {
					this.$axios({
						method: 'post',
						url: rts.$appUrl +`/api/expend/delete-expend`,
						headers: {
