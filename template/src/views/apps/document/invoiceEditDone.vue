            ...
            <el-row class="items mt-4" :gutter="20">
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb-10">
                    <el-card class="box-card" align="right">
                        <el-button type="success" @click="$router.push('/invoices')" plain>Хадгалах</el-button>
                    </el-card>
                </el-col>
    ...
    async beforeRouteLeave (to, from , next) {
          ...
          next();
			...
			if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/invoice/delete-changed-invoice`,
					headers: {
    ...
    methods: {
        ...
        getInvoice() {
			...
			if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/invoice/get-invoice-from-offer`,
					headers: {
						"Authorization": `Bearer ${token}`
					},
                    data: {
