                        ...
                        <div class="page-invoice">
                            <div class="invoice bg-white black-text">		
                                ...
                                <div class="invoice-bottom" v-for="(item, index) in products" :key="index">
                                    <el-button type="primary" icon="el-icon-upload2" circle :style="{'position': 'absolute', 'bottom': '10px', 'right': '70px'}" size="mini" @click="uploadImage(item.id)"></el-button>
                                    <el-button type="primary" icon="el-icon-picture-outline" circle :style="{'position': 'absolute', 'bottom': '10px', 'right': '40px'}" size="mini" @click="showBigImage(item.image, item.image1, item.image2, item.image3, item.image_type)"></el-button>
                                    <el-button type="danger" icon="el-icon-delete" circle :style="{'position': 'absolute', 'bottom': '10px', 'right': '10px'}" size="mini" @click="removeProduct(item, index)"></el-button>
                                    <table>
                                        <thead>
                                            <tr>
                                                <th>Бүтээгдэхүүн</th>
    ...
    methods: {
        ...
        onProfileImageSelected(event) {
            if(event.target.files[0].size / 1024 > 100) {
                this.$notify({
					title: 'Амжилтгүй',
					message: `100KB -аас ихгүй байх ёстой`,
					type: 'error'
				});
            } else {
                const file = event.target.files[0];
                this.upload.selected = file;
                var rts = this;
                const token = localStorage.getItem('token');
                if(this.upload.selected) {
                    const fd = new FormData();
                    fd.append('photo', this.upload.selected, this.upload.selected.name);
                    fd.append('id', JSON.stringify(this.upload.id));
                    this.$axios.post(this.$appUrl+'/api/offer/update-offer-product-image', 
                    fd,
                    {
                        headers: {
                    ...
                    .then(function(data) {
                        ...
                        rts.products.forEach(el => {
                            if(el.id == rts.upload.id) {
                                el.image = data.data.image;
                                el.image1 = 'no-image.jpg';
                                el.image2 = 'no-image.jpg';
                                el.image3 = 'no-image.jpg';
                                el.image_type = 2;
                            }
                        });
                    }).catch(error => {
        ...
        showBigImage(img, img1, img2, img3, type) {
            if(type == 1) {
                this.bigImageSource = [this.$imgUrl + img, this.$imgUrl + img1, this.$imgUrl + img2, this.$imgUrl + img3];
            } else {
                var u = this.$appUrl + '/images/local/offer/';
                this.bigImageSource = [u + img, u + img1, u + img2, u + img3];
            }
        ...
        addToInvoice(val) {
            ...
            if(ex) {
                ...
                if(token) {
                    this.$axios({
                        method: 'post',
                        url: rts.$appUrl +`/api/offer/update-offer-product`,
                        headers: {
                            "Authorization": `Bearer ${token}`
                        },
                        data: {
                    ...
                    .then(function(data){
                        if(data.data.result == 'success') {
                            var l = rts.products[rts.selectedIndex];
                            ...
                            l.image1 = val.image2;
                            l.image2 = val.image3;
                            l.offer_number = rts.detail.invoiceNumber;
                            l.image3 = val.image4;
                        }
                    }).catch(error => {
                        console.log(error);
        ...
        addProduct() {
            var arr = {product_name: '', product_model: '', remain: 1, price: 0, description: '', image: 'no-image.jpg', image1: 'no-image.jpg', image2: 'no-image.jpg', image3: 'no-image.jpg', image_type: 2, offer_number: this.detail.invoiceNumber};
            var rts = this;
			const token = localStorage.getItem('token');
            if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/offer/add-product-to-offer`,
					headers: {
						"Authorization": `Bearer ${token}`
					},
                    data: {
        ...
        removeProduct(p, index) {
             var rts = this;
			const token = localStorage.getItem('token');
            if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/offer/delete-offer-product`,
					headers: {
						"Authorization": `Bearer ${token}`
					},
                    data: {
                        offernumber: this.detail.invoiceNumber,
                        id: p.id
                    }
				})
				.then(function(data){
					if (data.data.result == 'success') {
                        rts.products.splice(index, 1);
					}
				}).catch(error => {
					console.log(error);
				});
