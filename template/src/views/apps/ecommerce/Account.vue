							...
							<div class="block">
								...
								<el-carousel-item v-for="(item, index) in checkPlan(data.day)" :key="index">
									...
									<div class="item-box item-status-calendar" align="center" :style="{'border-radius': '5px', 'margin-top': '3px'}" v-bind:class="{'status-Paid': 1 == 1}">
										<span v-if="item.second == 'none'">
											{{item.name}} <br> {{item.start}} - {{item.end}} 
										</span>
										<span v-else>{{item.name}} <br> {{item.start}} - {{item.second}} - {{item.end}}</span>
									</div>
								</el-carousel-item>
								</el-carousel>
							</div>
			...
			this.plan.forEach(el => {
				if(date == el.date) {
					if(new Date(date).getDay() === 6 || new Date(date).getDay() === 0) {
						shop.push({name: el.name, start: el.weekend, end: el.end, second: el.second_part});
					} else {
						shop.push({name: el.name, start: el.start, end: el.end, second: el.second_part});
					}
				}
			});
			return shop;
